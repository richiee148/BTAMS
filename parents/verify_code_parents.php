...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $email = $_SESSION['email']; // email stored during forgot_password step

    // Check if code matches the one in DB
    $stmt = $conn->prepare("SELECT id FROM users WHERE email = ? AND reset_code = ?");
    $stmt->bind_param("ss", $email, $code); // both email and reset_code as strings
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        header("Location: reset_password_parents.php");
        exit();
    } else {
...
<body>
   ...
   <div class="container-fluid vh-100 d-flex justify-content-center align-items-center">
     <div class="card p-5 shadow rounded-4 text-center login-card" style="max-width:500px; width:100%;">
       ...
       <form action="verify_code_parents.php" method="POST">
       ...
       <div class="text-center">
         <p class="small mb-0 mt-2">Go back to <a href="login_parents.php" class="text-decoration-none fw-bold">Login</a></p>
       </div>
     </div>
   </div>
