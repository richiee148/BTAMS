...
<div class="report-container">
  ...
  <div class="kpi-cards-grid">
    ...
    <div class="kpi-card actual-card">
      ...
      <div class="card-value" id="totalCollected">₱<?= number_format($totalActual, 2) ?></div>
    </div>
...
<script>
  function applyFilters() {
    ...
    rows.forEach(row => {
      ...
      const matchesYear = schoolYear === '' || rowYear === schoolYear;

      row.style.display = (matchesSearch && matchesCollege && matchesYear) ? '' : 'none';
    });
  }

  function filterBySchoolYear(year) {
    if (year) {
      document.getElementById('footer-note').textContent = 'Note: This dashboard is for viewing and transparency purposes only. Currently viewing School Year ' + year + '.';
    }
    applyFilters();
  }
